...
if(length(args) == 0) {
  ...
  cat("  --dir=<path>        Remote directory (default: auto-generated)\n")
  ...
  cat("Examples:\n")
  cat("  # Run single job type\n")
  cat("  Rscript launch.R model\n")
  cat("  Rscript launch.R hessian --nsplit=100\n\n")
  cat("  # Run multiple job types\n")
  cat("  Rscript launch.R model,hessian\n")
  cat("  Rscript launch.R model,hessian,prof,jitter\n")
  cat("  Rscript launch.R all                    # All jobs\n\n")
  cat("  # Run specific model(s)\n")
  cat("  Rscript launch.R model --model=base\n")
  cat("  Rscript launch.R hessian,prof --model=base,M1\n\n")
  cat("  # Local test (uses first model)\n")
...
if("model" %in% job_types) {
  s for", length(model_names), "model(s)...\n\n")
  ...
  for(model_name in model_names) {
    ...
    job_info_file <- file.path(get_model_dir(model_name), "condor_job_info.txt")
    cat("==============================================\n", file = job_info_file)
...
if("prof" %in% job_types) {
  ...
  cat("\n✓ Submitted all", nsplit, "hessian parts\n\n")
  cat("After completion, run:\n")
  cat("  Rscript collate_hessian_mfcl.R\n")
  cat("  Rscript verify_hessian.R\n")
}
 profile jobs for", length(model_names), "model(s)...\n")
  ...
  cat("\n✓ Submitted", length(model_names) * length(scalers_vec), "profile job(s)
      exclude_slots = exclude_slots,
      custom_batch_name = paste0("prof_sc", sc, "-", format(Sys.time(), "%H:%M:%S_%m%d")),
      condor_environment = prof_env
    )
  }
  
  cat("\n✓ Submitted all", length(scalers_vec), "profile jobs\n")
}

## ============== jitter jobs for", length(model_names), "model(s)...\n")
  ...
  cat("\n✓ Submitted all", njitter, "jitter runs\n")
}
...
cat("    folder_name = '", GITHUB_REPO, "/", remote_dir, "',\n", sep="")
cat("    action = 'fetch',\n")
cat("    fetch_dir = 'model',\n")
