...
generate_proflike_script <- function(
  ...
  # Same switch generator as original
  generate_switch <- function(reps, target, weight, quant_type, af172, af173, af174) {
  ...
  bash_script <- c(
    ...
    '    echo "file $4 does not exist"',
    sprintf("    $1 $2 $3 $4 \\"),
    sprintf("    %s", generate_switch("$8", "$Target", "$7", QuantityType, "$9", "${10}", "${11}")),
    "  else",
    '    echo "file $4 exists already"',
    "  fi",
    "}",
    ...
    sprintf("    %s", generate_switch("1", "0", "0", QuantityType, "$Af172", "$Af173", "$Af174")),
    "}",
    "",
    ...
    sprintf("  call_mf1 $PROG_PATH $Frq $Initp ${QUANTITY_LABEL}${M0}a.par $M0 $MLE $Pen1 $Reps1 $Af172 $Af173 $Af174"),
    "else",
