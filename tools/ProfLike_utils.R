...
generate_proflike_script <- function(
  ...
  bash_script <- c(
    ...
    "  else",
    "    # Indepvar fixed-parameter profile: the target parameter is already",
    "    # injected into Initp before this script runs, and the chain runner",
    "    # warm-starts Initp from the previous scalar's final par.  A single",
    "    # optimizer call is therefore sufficient -- no warmup pass needed.",
    ...
    "      $PROG_PATH $Frq $Initp ${QUANTITY_LABEL}${Mult}finalz.par $FINAL_SWITCH_INDEP",
    "    fi",
    "    lock_indepvar_par ${QUANTITY_LABEL}${Mult}finalz.par",
    "  fi",
