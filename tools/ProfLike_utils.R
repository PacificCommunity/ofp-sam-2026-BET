...
generate_proflike_script <- function(
    ...
    ExtraFarRefine = TRUE,
    MaxGradientAccept = 0.001,
    MaxGradientRetries = 2,
    filename = "ProfLike.sh") {
  ...
  bash_script <- c(
    ...
    sprintf("MAX_GRAD_ACCEPT=%.6f", as.numeric(MaxGradientAccept)),
    sprintf("MAX_GRAD_RETRIES=%d", as.integer(MaxGradientRetries)),
    "",
    "# Function matching original call_mf1",
    "function call_mf1 () {",
    '  echo "arg5=$5"',
    ...
    "}",
    "",
    "extract_max_gradient() {",
    "  if [ -f sorted_gradient.rpt ]; then",
    "    tail -n 1 sorted_gradient.rpt | awk '{print $2}'",
    "  elif [ -f gradient.rpt ]; then",
    "    sed -n '2p' gradient.rpt | awk '{m=0; for(i=1;i<=NF;i++){v=$i+0; if(v<0)v=-v; if(v>m)m=v} END{print m+0}'",
    "  else",
    "    echo NA",
    "  fi",
    "}",
    ...
    "}",
    "",
    "refine_until_gradient_ok() {",
    "  local mult=$1",
    "  local current_par=$2",
    "  local attempt=1",
    "  local grad=$(extract_max_gradient)",
    "  ",
    "  if [ \"$grad\" = \"NA\" ]; then",
    "    echo \"No gradient report found after $current_par; skipping max-gradient check\"",
    "    echo \"$current_par\"",
    "    return 0",
    "  fi",
    "  ",
    "  echo \"Final max gradient for $current_par: $grad\"",
    "  while awk -v g=\"$grad\" -v t=\"$MAX_GRAD_ACCEPT\" 'BEGIN{exit !(g >= t)}'; do",
    "    if (( attempt > MAX_GRAD_RETRIES )); then",
    "      echo \"Max gradient still >= $MAX_GRAD_ACCEPT after $MAX_GRAD_RETRIES retries; keeping $current_par\"",
    "      break",
    "    fi",
    "    local next_par=${QUANTITY_LABEL}${mult}grad${attempt}.par",
    "    echo \"Gradient $grad >= $MAX_GRAD_ACCEPT; refinement attempt $attempt -> $next_par\"",
    "    call_mf1 $PROG_PATH $Frq $current_par $next_par $mult $MLE $CUR_PEN4 $CUR_REPS6 $Af173 $Af174",
    "    current_par=$next_par",
    "    grad=$(extract_max_gradient)",
    "    if [ \"$grad\" = \"NA\" ]; then",
    "      echo \"No gradient report found after $current_par; stopping extra refinement\"",
    "      break",
    "    fi",
    "    echo \"Updated max gradient for $current_par: $grad\"",
    "    attempt=$(( attempt + 1 ))",
    "  done",
    "  echo \"$current_par\"",
    "}",
    ...
    "  call_mf1 $PROG_PATH $Frq $Initp ${QUANTITY_LABEL}${Mult}a.par $Mult $MLE $CUR_PEN1 $CUR_REPS1 $Af173 $Af174",
    "  call_mf1 $PROG_PATH $Frq ${QUANTITY_LABEL}${Mult}a.par ${QUANTITY_LABEL}${Mult}b.par $Mult $MLE $CUR_PEN2 $CUR_REPS2 $Af173 $Af174",
    "  call_mf1 $PROG_PATH $Frq ${QUANTITY_LABEL}${Mult}b.par ${QUANTITY_LABEL}${Mult}c.par $Mult $MLE $CUR_PEN3 $CUR_REPS3 $Af173 $Af174",
    "  call_mf1 $PROG_PATH $Frq ${QUANTITY_LABEL}${Mult}c.par ${QUANTITY_LABEL}${Mult}final.par $Mult $MLE $CUR_PEN3 $CUR_REPS4 $Af173 $Af174",
    "  call_mf1 $PROG_PATH $Frq ${QUANTITY_LABEL}${Mult}final.par ${QUANTITY_LABEL}${Mult}finalx.par $Mult $MLE $CUR_PEN3 $CUR_REPS5 $Af173 $Af174",
    "  call_mf1 $PROG_PATH $Frq ${QUANTITY_LABEL}${Mult}finalx.par ${QUANTITY_LABEL}${Mult}finaly.par $Mult $MLE $CUR_PEN3 $CUR_REPS6 $Af173 $Af174",
    "  if (( CUR_EXTRA_REFINE == 1 )); then",
    "    call_mf1 $PROG_PATH $Frq ${QUANTITY_LABEL}${Mult}finaly.par ${QUANTITY_LABEL}${Mult}finalzz.par $Mult $MLE $CUR_PEN4 $CUR_REPS4 $Af173 $Af174",
    "    call_mf1 $PROG_PATH $Frq ${QUANTITY_LABEL}${Mult}finalzz.par ${QUANTITY_LABEL}${Mult}finalz.par $Mult $MLE $CUR_PEN4 $CUR_REPS6 $Af173 $Af174",
    "  else",
    "    call_mf1 $PROG_PATH $Frq ${QUANTITY_LABEL}${Mult}finaly.par ${QUANTITY_LABEL}${Mult}finalz.par $Mult $MLE $CUR_PEN3 $CUR_REPS4 $Af173 $Af174",
    "  fi",
    "  FINAL_PAR=$(refine_until_gradient_ok $Mult ${QUANTITY_LABEL}${Mult}finalz.par | tail -n 1)",
    "  echo \"Accepted final par for scalar $Mult: $FINAL_PAR\"",
    "done",
    "",
    'echo "========================================"',
    'echo "Likelihood profile calculation complete"',
  ...
  cat("  Max gradient accept:", MaxGradientAccept, "\n")
  cat("  Max gradient retries:", MaxGradientRetries, "\n")
}
