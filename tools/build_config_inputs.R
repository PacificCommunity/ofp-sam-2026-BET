...
rows <- lapply(names(models), function(model_name) {
  ...
  data.frame(
    ...
    base_source = if (!is.null(m$input_recipe_base_source)) as.character(m$input_recipe_base_source) else "",
    ...
    release_regions = if (!is.null(m$input_recipe_release_regions)) as.character(m$input_recipe_release_regions) else "9",
    with_11par = if (!is.null(m$input_recipe_with_11par)) as.character(m$input_recipe_with_11par) else "1",
    program_path = if (!is.null(m$program_path)) as.character(m$program_path) else "",
...
for (idx in seq_len(nrow(plan))) {
  ...
  cmd_args <- c(
    "tools/build_4region_input_recipe.R",
    "--output-dir", p$output_dir,
    "--base", p$base,
    "--release-regions", p$release_regions
  )
  if (nzchar(p$base_source)) cmd_args <- c(cmd_args, "--base-source", p$base_source)
  ...
  if (nzchar(p$program_path)) cmd_args <- c(cmd_args, "--program-path", p$program_path)
  if (as_flag(p$index_cv_half)) cmd_args <- c(cmd_args, "--index-cv-half")
  if (as_flag(p$with_11par)) cmd_args <- c(cmd_args, "--with-11par")
