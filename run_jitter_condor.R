#!/usr/bin/env Rscript
## Launch jitter analysis jobs on Condor
...
## Jitter settings
njitter <- 100  # Number of jitter runs

## Remote directory (optional - auto-generated if NULL)
remote_dir <- NULL
# remote_dir <- "develop/test_jitter"
...
cat("\n")
cat("==============================================\n")
cat("LAUNCHING JITTER ANALYSIS JOBS\n")
cat("==============================================\n")
cat("Models:", paste(model_names, collapse = ", "), "\n")
cat("Runs per model:", njitter, "\n")
cat("Total jobs:", length(model_names) * njitter, "\n")
...
if(run_local) {
  cat("Running local test: model", model_names[1], "seed 1\n\n")
  ...
  cat("\n✓ Local test complete\n")
  cat("Check: model/", model_names[1], "/jitter/\n\n", sep = "")
...
## Build command
cmd <- c("Rscript", "launch.R", "jitter",
         paste0("--model=", paste(model_names, collapse = ",")),
         paste0("--njitter=", njitter),
...
cat("  Rscript fetch_results.R ", remote_dir, " --job=jitter --njitter=", njitter, "\n", sep = "")
cat("  # or\n")
cat("  ./fetch.sh ", remote_dir, " --job=jitter\n\n", sep = "")
