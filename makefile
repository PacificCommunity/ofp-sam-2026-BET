...
SELFTEST_MODEL_DIR ?= model/$(notdir $(SELFTEST_BASE_DIR))
SELFTEST_SOURCE_PAR ?=
SELFTEST_SOURCE_MODE ?= last_par
SELFTEST_REFIT_MODE ?= last_par
SELFTEST_DIR ?= $(SELFTEST_MODEL_DIR)/selftest
...
build-input-recipe:
	...
	Rscript tools/input_sensitivities/build_input_recipe.R \
		...
		--fixed-params "$(INPUT_RECIPE_FIXED_PARAMS)" \
...
selftest_build:
	...
	selftest_model_dir="$(SELFTEST_MODEL_DIR)" \
