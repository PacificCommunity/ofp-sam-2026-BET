...
mod_likelihood_server <- function(input, output, session, rv) {
  ...
  lik_filters_current <- reactive({
    ...
    legend_mode <- sanitize_profile_type(input$lik_legend_mode, allowed = c("top", "all", "none"), default = "all")
  ...
  lik_plot_cache_key <- reactive({
    ...
    key$legend_mode <- filters$legend_mode
    key$legend_top_n <- filters$legend_top_n
    key$legend_deemphasize_others <- isTRUE(filters$legend_deemphasize_others)
    if (identical(filters$profile_type, "tagging")) {
      key$tagging_view <- filters$tagging_view
  ...
  observe({
    ...
    sel_scenarios <- input$lik_scenarios
    scenarios <- unique(c(
      as.character(sel_scenarios),
      names(rv$ParOut_list),
      names(rv$IndepOut_list)
    ))
      ...
      if (any(has_payload)) {
        ...
        if (length(payloads) > 0) {
          ...
          has_fix_details_in_payloads <- any(vapply(seq_along(payloads), function(i) {
            dd <- payloads[[i]]$indepvar_fix_details
            if (!is.data.frame(dd) || nrow(dd) == 0) {
              dd <- valid_infos[[as.character(existing_scales[[i]])]]$indepvar_fix_details
            }
            is.data.frame(dd) && nrow(dd) > 0
          }, logical(1)))
          if (isTRUE(indepvar_fix_applied) || has_fix_details_in_payloads) {
          ...
          if (!isTRUE(indepvar_fix_applied) && has_fix_details_in_payloads) {
            indepvar_fix_applied <- TRUE
            if (is.na(use_quantity_penalty)) use_quantity_penalty <- FALSE
          }
  ...
  profile_conflict_tables_reactive <- reactive({
    ...
    scenario_change_tokens <- lapply(unique(summary_base$scenario), function(sc) {
      ...
      meta <- sp_model_change_metadata(file.path(input$model_dir, sc), sc, info)
      tokens <- meta$tokens
      source <- meta$source
  ...
  likelihood_plot_reactive <- reactive({
    ...
    if (identical(plot_kind, "jitter_params")) {
        ...
        if (metric %in% c("delta", "pct_change", "baseline_minus", "rel_baseline_minus", "bound_delta", "sampler_shift", "rel_diff") && range_pct < 100) {
          if (length(finite_vals) > 0) {
            ...
            if (is.finite(robust_limit) && robust_limit > 0) {
              ...
              } else if (identical(metric, "bound_delta")) {
                plot_limit <- c(max(plot_limit[1], -1), min(plot_limit[2], 1))
              }
            }
          }
        }
      ...
      y_label <- case_when(
        ...
        metric == "bound_delta" ~ "Jittered Input Parameter Bound-normalised Displacement",
  ...
  output$profile_conflict_table_ui <- renderUI({
    ...
    box(
      ...
      div(
        ...
        tags$div("Change diagnostics use model/input metadata tokens first; model-name parsing is only a conservative known-token fallback for older outputs.", style = "font-size: 12px; color: #333; margin-top: 4px;"),
