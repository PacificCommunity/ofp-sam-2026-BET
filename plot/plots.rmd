...
results <- list.dirs(file.path(getwd(), "..", "model"), full.names = TRUE, recursive = FALSE) %>%
  purrr::map(function(folder) {
    ...
    list(ParOut = ParOut, RepOut = RepOut)
  })
...
## Spawning depletion
```{r, echo=FALSE, message=F, fig.dim=c(8,6)}
...
# Ensure consistent scenario ordering in SBdep and Rec
SBdep <- SBdep %>%
...
# Create a shared legend plot with a dummy dataset
legend_data <- data.frame(
  Year = 1:length(scenario_colors),  # Dummy x values
  Quant = 1:length(scenario_colors),  # Dummy y values
  Scenario = factor(names(scenario_colors), levels = names(scenario_colors))  # Use all scenarios
)

shared_legend <- ggplot(legend_data, aes(x = Year, y = Quant, color = Scenario)) +
  geom_line(linewidth = linewidth, alpha = alpha) +  # Match alpha and linewidth
  scale_color_manual(values = scenario_colors) +
  theme(
  ...
  labs(color = "Scenario")  # Add legend title

# Extract the legend
legend <- cowplot::get_legend(shared_legend)

# Combine the plots and add the shared legend
...
final_plot <- cowplot::plot_grid(
  ...
  ncol = 2,
  rel_widths = c(4, 1)  # Adjust widths for plot and legend
)
