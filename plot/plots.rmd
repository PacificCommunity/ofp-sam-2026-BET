...
results <- list.dirs(file.path(getwd(), "..", "model"), full.names = TRUE, recursive = FALSE) %>%
  purrr::map(function(folder) {
    ...
    list(ParOut = ParOut, RepOut = RepOut, LikOut=LikOut_list, LikRawOut=LikRawOut_list)
  })
...
# 
# # Filtered fisheries
# fisheries <- names(fisheries_mapping)
...
<!-- ```{r model_profile,  eval=ProfLike, echo=FALSE, message=F, fig.dim=c(12,8)} -->
...
<!-- LikeCompScalers=list() -->

<!-- LikeComp100=sapply(scenarios_name, function(x) c("Indices"=sum(results[[x]]$LikOut@survey_index), -->
<!--                                                  "LFs"=sum(ProfLikOut_list[[x]]@total_length_fish), -->
<!--                                                  "Penalties"=as.numeric(ProfLikRawOut_list[[x]][pen_idx:(pen_idx + 1)][2]), -->
<!--                                                  #"Weight comp"=sum(LikOut_list[[x]]@total_weight_fish), -->
<!--                                                  "Tags"=sum(unlist(ProfLikOut_list[[x]]@tag_rel_fish, recursive = T)) ) ) -->

<!-- LikeComp100=rbind(LikeComp100,  "Total"=apply(LikeComp100, 2, sum)) -->



<!-- for (i in scalers) { -->
<!--   LikeCompScalers[[as.character(i)]]=sapply(scenarios_name, function(x) c("Indices"=sum(ProfLikOut_list[[x]][[as.character(i)]]@survey_index), -->
...
<!--   LikeCompScalers[[as.character(i)]]=rbind(LikeCompScalers[[as.character(i)]],  "Total"=apply(LikeCompScalers[[as.character(i)]], 2, sum) -->
<!--   ) -->
...
<!-- LikeCompAll=c(list("100"=LikeComp100), LikeCompScalers) -->

<!-- LikeComp_df <- purrr::imap_dfr(LikeCompAll, function(df, indicator) { -->
<!--   as.data.frame(df) %>% -->
<!--     tibble::rownames_to_column("Likelihood") %>% -->
<!--     mutate(scaler = indicator) -->
<!-- }) -->

<!-- LikeComp_long <- LikeComp_df %>% -->
<!--   pivot_longer( -->
<!--     cols = scenarios_name, -->
<!--     names_to = "scenario", -->
<!--     values_to = "value" -->
<!--   ) %>% -->
<!--   mutate(scaler = as.numeric(scaler)*0.01*unlist(sapply(scenarios_name, function(x) read.table(paste0("../analysis/scenarios/",x, "/ProfLike/avg_bio")))) -->
<!-- ) -->


<!-- LikeComp_change <- LikeComp_long %>%  -->
...
<!-- LikeComp_change %>% filter(Likelihood != "Penalties") %>% -->
